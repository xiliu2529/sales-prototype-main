...
const SchemaUtils = {
  ...
  getTableSchema(tableName, schema) {
    ...
    schema.forEach((field) => {
      ...
      // 字符出现的次数
      let index = field.title.indexOf('.'); // 字符出现的位置
      let num = 0; // 这个字符出现的次数
      while(index !== -1) {
        num+=1; // 每出现一次 次数加一
        index = field.title.indexOf('.',index + 1); // 从字符串出现的位置的下一位置开始继续查找
      }

      if(num === 0){
        newCols.push(this.getHeader(field));
      }else if(num === 1){
        newCols = this.getOneCommaHeader(newCols,field)
      }else if(num === 2){
        newCols = this.getTwoCommaHeader(newCols,field)
      }
      else if(num === 3){
        newCols = this.getThreeCommaHeader(newCols,field)
      }
    });
  ...
  getHeader(field){
    ...
    col.key = field.fieldClm;
    // 用key作为 dataIndex
    col.dataIndex = field.fieldClm;
    if(field.title !== '単価（千元/千円）'){
    ...
    col.children = field.children;
    col.defaultValue = field.defaultValue;
    col.disable = field.disable;
    return(col);
  },
   ...
   * @param field
   */
  getOneCommaHeader(newCols, field){
    // 第一次加入父节点，第二次只需吧子节点加入到父节点中即可
    const newColsParam = newCols.filter((item)=>item.title===field.title.substr(0,field.title.indexOf('.',1)))

    if(newColsParam.length ===0 ){
      const col = {};
      col.title = field.title.substr(0,3).replace('.','');
      col.children = [];

      const subCol = {};
      subCol.index = field.index;
      subCol.key = field.fieldClm;
      // 用key作为 dataIndex
      subCol.dataIndex = field.fieldClm;
      subCol.title = field.title.substr(3).replace('.','');
      subCol.fixed = field.fixed;
      subCol.width = field.width+'px';
      subCol.align = 'center';
      subCol.dataAlign = field.align;
      subCol.sorter = field.sorter;
      subCol.dataType = field.dataType;
      subCol.defaultValue = field.defaultValue;
      subCol.disable = field.disable;
    ...
    newColsParam[0].children.push(subCol);
    return(newCols);
  },
   ...
   * @param field
   */
  getTwoCommaHeader(newCols, field){
    // 找到最上面的节点
    const newColsParam = newCols.filter((item)=>item.title===field.title.substr(0,field.title.indexOf('.',1)))
    ...
    // 用key作为 dataIndex
    subCol.dataIndex = field.fieldClm;
    subCol.title = field.title.substr(field.title.indexOf('.',2)).replace('.','');
    subCol.fixed = field.fixed;
    subCol.width = field.width+'px';
    subCol.align = 'center';
    subCol.dataAlign = field.align;
    ...
    newColsParam[0].children.children.push(subCol);
    return(newCols);
  },
   ...
   * @param field
   */
  getThreeCommaHeader(newCols, field){
    // 找到最上面的节点
    const newColsParam = newCols.filter((item)=>item.title===field.title.substr(0,field.title.indexOf('.',1)))
    ...
    // 用key作为 dataIndex
    subCol.dataIndex = field.fieldClm;
    subCol.title = field.title.substr(field.title.indexOf('.',3)).replace('.','');
    subCol.fixed = field.fixed;
    subCol.width = field.width+'px';
    subCol.align = 'center';
    subCol.dataAlign = field.align;
    ...
    newColsParam[0].children.children.children.push(subCol);
    return(newCols);
  },
  ...
  createForm(tableName, schema) {
    ...
    const schemaCallback = this.parseFormSchema(schema);
    if (!ignoreCache) {
