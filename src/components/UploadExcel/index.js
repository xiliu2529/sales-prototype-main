...
class DBTable extends React.Component {
  ...
  state = {
    // 本身的状态
    loadingSchema: true,  // 是否正在从远程加载schema
...
/*    const {action} = this.props.location;
    ...
    this.tryFetchSchema(nextProps, (res) => {
      ...
      this.setState({
        ...
        // data: nextProps.excelData[0].filter((item)=>item.No <= this.state.pageSize),
        tableLoading: false,
        currentPage: 1,
        // total: nextProps.excelData[0].length,
        loadingSchema: false,
      },
      //  this.refresh
      );
  ...
  async tryFetchSchema(props, callback) {
    ...
    // 这里注意, 区分同步/异步
    let dataSchema = TableUtils.getCacheSchema(tableName);
    if (!dataSchema) {
      if (tableConfig.asyncSchema === true) {
        // 如果是异步的, 必须给用户一个loading提示
        this.state.loadingSchema = true;
        // tmp = await TableUtils.getExcelSchema(tableName, this.props.excelSchema);
      } else {
        // 获取本地schema
        dataSchema = TableUtils.getLocalSchema(tableName);
      }
    }

    const res = {dataSchema, tableName, tableConfig};
    callback(res);
  }
...
/*    if (input.querySchema) {
  ...
  refresh = async() => {
    ...
    this.setState({
      // total: this.props.excelData[0].length,
      // eslint-disable-next-line react/no-access-state-in-setstate
      // data:this.props.excelData[0].filter((item)=>item.No <= this.state.pageSize),
      tableLoading: false,
    })
...
/*    const res = await this.select(this.state.queryObj, this.state.currentPage, this.state.pageSize);
  ...
  handlePageChange = async(page) => {
    this.setState({
      currentPage: page,
      // eslint-disable-next-line react/no-access-state-in-setstate
      data: this.props.excelData[0].filter((item)=>item.No > (page-1) * this.state.pageSize && item.No < page * this.state.pageSize + 1),
      tableLoading: false,
    });
/*    logger.debug('handlePageChange, page = %d', page);
  ...
  handleShowPageChange = async(page, pageSize) => {
    this.setState({
      currentPage: page,
      data: this.props.excelData[0].filter((item)=>item.No > (page-1) * pageSize && item.No < page * pageSize + 1),
      tableLoading: false,
      pageSize: pageSize,
    });
    ...
    return (
      <Spin spinning={this.state.loadingSchema} delay={500}>
        <InnerTable
          ...
          schema={this.props.headerSchema}
          refresh={this.refresh}
          tableConfig={this.tableConfig}
          tableName={this.tableName}
          year={this.props.year}
          orgGroupId={this.props.orgGroupId}
          effortUnitCd={this.props.effortUnitCd}
          dispatch={this.props.dispatch}
          loginUserCd={this.props.loginUserCd}
          userList={this.props.userList}
          amtRt={this.props.amtRt}
          procName={this.props.procName}
          modelVisible={this.props.modelVisible}
          message={this.props.message}
          loadDataLoading={this.props.loadDataLoading}
        />
