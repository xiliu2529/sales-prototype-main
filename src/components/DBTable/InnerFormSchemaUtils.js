...
const SchemaUtils = {
  ...
  createForm(tableName, schema) {
    ...
    class tmpComponent extends React.Component {
      ...
      render() {
        ...
        return this.schemaCallback(Form.Item.getFieldDecorator);
      }
    }
    // 注意要再用antd的(create)方法包装下
    return tmpComponent;
  },
  ...
  transformCascader(field) {
    ...
    return this.colWrapper( <Cascader options={field.options} expandTrigger="hover" placeholder={field.placeholder || '请选择'} size="default"/>, field);
  },
  ...
  transformNormal(field) {
    switch (field.dataType) {
      case 'int':
        ...
        return this.colWrapper( <InputNumber size="default" max={field.max} min={field.min} placeholder={field.placeholder}/>,field);
      case 'float':
        logger.debug('transform field %o to float input component', field);
        return this.colWrapper( <InputNumber step={0.01} size="default" max={field.max} min={field.min} placeholder={field.placeholder}/>, field);
      case 'datetime':
        logger.debug('transform field %o to datetime input component', field);
        return this.colWrapper( <DatePicker showTime format="YYYY-MM-DD HH:mm:ss" placeholder={field.placeholder || '请选择日期'}/>, field);
      default:  // 默认就是普通的输入框
        logger.debug('transform field %o to varchar input component', field);
        return this.colWrapper( <Input placeholder={field.placeholder} size="default" addonBefore={field.addonBefore} addonAfter={field.addonAfter}/> , field);
    }
  },
  ...
  transformBetween(field) {
    ...
    switch (field.dataType) {
      case 'int':
        ...
        beginFormItem = <InputNumber size="default" placeholder={field.placeholderBegin || '最小值'}/>;
        endFormItem =   <InputNumber size="default" placeholder={field.placeholderEnd || '最大值'}/>;
      ...
      case 'float':
        ...
        beginFormItem = <InputNumber step={0.01} size="default" placeholder={field.placeholderBegin || '最小值'}/>;
        endFormItem = <InputNumber step={0.01} size="default" placeholder={field.placeholderEnd || '最大值'}/>;
        return this.betweenColWrapper(beginFormItem, endFormItem, field);
